...
def searchres(engine, keyword):   
    result = []
    with engine.connect() as connection:
        getDetails = connection.execute(text("SELECT pollId FROM polls WHERE (LOWER(pollname) LIKE :keyword OR LOWER(pollDescription) LIKE :keyword OR LOWER(pollQuestion) LIKE :keyword) ORDER BY pollStart LIMIT 50"), {'keyword': "%" + keyword.lower() + "%"})
